\section{Analysis of the Cox-Ross-Rubinstein model}
In this section, we will examine the theory behind the Cox-Ross-Rubinstein model to implement it later in the next section.

\subsection{Objectives and Structure of the Model}

The objective of the \textbf{binomial} or \textbf{Cox-Ross-Rubinstein} model is to find the no-arbitrage price of the derivative product.
The \textbf{no-arbitrage price} is the price at which no arbitrage opportunity exists.
This model is based on portfolio replication, the absence of arbitrage and market completeness to exactly replicate the option's future cash flows and to guarantee the existence and uniqueness of a risk-neutral probability measure.


The \textbf{Cox-Ross-Rubinstein} model is a discrete pricing model unlike the \textbf{Black-Scholes} model which is continuous.
The main advantages of working in discrete time are the ease of modeling and algorithmic implementation.
Indeed, the binomial model is intuitively easy to understand as it represents asset evolution along the branches of a binary tree weighted by the probabilities of price increases and decreases.
We will focus on this representation in the remainder of this section.

To discretize the time horizon of the call, we consider subdivision of the option's validity period into $N$ periods of equal length, yielding the following divisions :

$$t_n=\frac{nT}{N} ,\forall n \in {0,..,N}$$

\remark Letting $N$ tend towards very large values amounts to approaching a continuous model.

\bigskip

For each subdivision, two factors are defined :
\begin{center}
    \begin{itemize}
        \item[\textbf{u}] : the asset's upward movement factor (up)
        \item[\textbf{d}] : the asset's downward movement factor (down)
    \end{itemize}
\end{center}
These two factors correspond to the rise and fall in the asset's price, respectively.
In a tree diagram, we will consider the convention of placing the up-factor on the upper branch and vice-versa.

\remark For the time being, we won't dwell on how these two factors are calculated.

The algorithm therefore breaks down into two distinc phases :
\begin{itemize}
    \item \textbf{The propagation phase (Forward Phase) :} Starting from $S_0$ the underlying asset price tree is constructed up to maturity $T$.
    \item \textbf{The evaluation phase (Backward Phase) :} Once the prices are set at maturity, the \textbf{payoffs} on the tree leaves are calculated.
    Then, we go backwards in the tree traversal to determine the payoff $S_0$
\end{itemize}
...
\remark L'arbre est recombinant, les évolutions sont invariantes par permutations de facteurs de hausse et baisse.  \\
...
L'idée du modèle de Cox-Ross-Rubinstein (et de celui de Black-Scholes) est d'utiliser la réplication de portefeuilles pour reproduire l'évolution d'un produit dérivé. En effet, on a supposé l'A.O.A donc le marché est complet et ainsi tout payoff est réplicable comme vu précédemment. \\
...
\begin{center}
\begin{tikzpicture}
    \node[malnode] (init) at (0,0) {$\Delta S + \beta B$};
    \node[malnode] (up) at (3, 1.5) {$\Delta uS + \beta BR$};
    \node[malnode] (down) at (3, -1.5) {$\Delta dS + \beta BR$};
...
\begin{align*}
    \left\{
    \begin{array}{cl}
        \Delta uS + \beta BR &= C_u \\
        \Delta dS + \beta BR &= C_d
    \end{array}
    \right.
\end{align*}
De plus,
\begin{align*}
    ...
    \end{array} \right. \iff  
    \left\{ \begin{array}{cl}
    -\Delta udS - d\beta BR&=-dC_u \\
    \Delta udS + u\beta BR&=uC_d \\
    \end{array} \right. \\
    \implies \beta RB(u-d)=uC_d-dC_u
    \implies \beta B=\frac{uC_d-dC_u}{R(u-d)}
\end{align*}
En remplaçant,
\begin{align*}
    C = \Delta S + \beta B &= \frac{C_u - C_d}{S(u-d)}S + \frac{uC_d - dC_u}{R(u-d)} \\
    &= \frac{R(C_u - C_d) + uC_d - dC_u}{R(u-d)} \\ 
...
\remark On a bien $1 = p + (1 - p) = \dfrac{R-d+u-R}{u-d} = 1$.\\ On a aussi nécessairement $d<R<u$. 
En effet, 
\begin{description}
...
Les valeurs des feuilles sont données par la formule du call (resp. put) si on modélise un call (resp. put). Ici, on aura : $C_{i,N} = \max(S_{i,N}-K,0)$ avec $S$ le prix du sous-jacent et $K$ le strike. 
\vspace{0.02\textheight}\\
Appliquons le modèle binomial sur une option dans l'exemple suivant: \\
\vspace{0.01\textheight}
\underline{Données} : 
\begin{itemize}
    \item Prix actuel : $S_0 = 100$
...
Pour l'instant nous nous sommes contentés de modéliser le prix d'options dites européennes, c'est-à-dire qu'elles ne sont exerçables qu'à la date d'échéance.
La force du modèle binomial réside dans le fait qu'il est facile de l'adapter aux options dites américaines. Une option américaine, contrairement à une option européenne,
peut être exercée à tout moment et pas obligatoire à échéance. On pourrait aussi étudier les options bermudéennes qui sont exerçables seulement à certaines dates définies avant échéance, 
mais par simplicité, nous étudierons seulement les options américaines dans cette partie.
Du fait de sa flexibilité, une option américaine a au moins la même valeur qu'une option européenne. En effet, une option américaine fait tout ce que fait une option
européenne mais peut être exercée plus tôt, ce qui permet d'éviter une baisse très importante, par exemple pour un call américain. 
\vspace{0.1cm} \\
En supposant qu'un investisseur est un être rationnel, c'est-à-dire qu'il va toujours choisir de prendre la décision pour maximiser sa rentabilité, il devient facile d'adapter notre modèle.
Il suffit de faire le maximum entre la valeur donnée par le modèle et la valeur d'exercice lors de la remontée de l'arbre. Par conséquent, un call américain et européen auront la même
valeur alors qu'un put américain sera plus cher qu'un put européen car le possesseur d'un put américain peut exercer son option pour éviter que le prix ne continue de chuter en dessous.
d'un certain niveau.\\

Formule Européenne : On actualise juste l'espérance de la période suivante.
$$C = \frac{1}{R}[ p C_u + (1-p) C_d]$$
Formule Américaine : À chaque nœud, on compare cette valeur actualisée avec le gain d'un exercice immédiat et on prend le maximum des deux.
$$C = \max ( \text{Valeur Intrinsèque} \, , \, \frac{1}{R} [ p C_u + (1-p) C_d ] )$$