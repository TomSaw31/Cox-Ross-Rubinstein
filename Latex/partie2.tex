...
\end{itemize}
~\\
Soit l'ensemble discret des temps $I = \{0,1,...,T\}, \: T < \infty$ où $T$ est la date de maturité de l'option. \\
...
$t\in I$. \\
\remark Pour chaque $t \in I$, la variable aléatoire $B_t$ est constante sur $\Omega$ ($\forall \omega \in \Omega, B_t(\omega)=(1+r)^t$. C'est donc une variable aléatoire déterministe. \\
...
\remark Soit $r$ le taux sans risque et un instant $t$.\\
A l'instant $t+1^-$, $\Pi^-_{t+1} = \Delta_tS_{t+1}+\beta_tB_{t+1}$ car la valeur de l'actif risqué $S_t$ est mise à jour et l'actif sans risque $B_t$ a produit des intérêts. La quantité d'actifs $\Delta$ ne varie pas. \\
A l'instant $t+1^+$, $\Pi^+_{t+1} = \Delta_{t+1}S_{t+1}+\beta_{t+1}B_{t+1}$ car on met à jour les quantités $\Delta_t$ et $\beta_t$ en achetant/vendant.\\
...
\begin{definitionbox}[frametitle = {Propriété d'autofinancement}]
    ...
    \begin{align*}
        \forall t \in \mathbb{N},  \quad \Pi_{t+1} = \Delta_t S_{t+1} + \beta_tB_{t+1} = \Delta_{t+1}S_{t+1} + \beta_{t+1}B_{t+1}
    \end{align*}
    où $r > -1$ est le taux sans risque. \cite{SelfFinancingPortfolio}
\end{definitionbox}
\vspace{0.025\textheight}
La propriété d'autofinancement permet donc d'assurer que l'évolution de $\Pi_t$ est entièrement déterminée par la stratégie $(\Delta_t,\beta_t)$ et la dynamique de $S_t$ et $B_t$ sans modifications externes. On considérera par la suite que tous les portefeuilles sont autofinancés.
\vspace{0.025\textheight}\\
...
\end{definitionbox}
\remark On déduit naturellement que la complétude d'un marché implique qu'il existe un portefeuille de réplication pour chaque payoff. On peut cependant aller plus loin et montrer l'unicité du portefeuille pour chaque payoff. \\
...
Alors $\Pi_T=\Pi^1_T-\Pi^2_T=H-H=0$ \\
Supposons que $\Pi_0 \neq 0$.  
Alors $\Pi$ constitue une opportunité d'arbitrage (gain certain à maturité sans investissement initial ou inversement en prenant $-\Pi$), ce qui contredit l'absence d'arbitrage.

Donc $\Pi_0 = 0$.

Comme $\Pi$ est autofinancé et de valeur finale nulle, l'absence d'arbitrage implique que $\Pi_t = 0$ pour tout $t$.
\pagebreak
...
\begin{definitionbox}[frametitle = {Théorèmes Fondamentaux de l'évaluation d'actifs}]
    \textbf{Premier Théorème : } Un marché financier sur un espace probabilisé discret $(\Omega,\mathcal{F},\mathbb{P})$ est sans opportunité d'arbitrage si et seulement s'il existe au moins une mesure de probabilité \textbf{risque-neutre} qui est équivalente à la mesure de probabilité originale $\mathbb{P}$.
...
\begin{definitionbox}[frametitle = {Martingale}]
    $M \coloneq (M_t)_{t \in I}$ est une $\mathcal{F}$-martingale si : 
    \begin{itemize}
        \item $\forall t \in I, M_t$ est $\mathcal{F}_t$-mesurable
        \item $\forall t \in I, M_t$ est intégrable
        \item $\forall s \le t$, $M_{s} = \mathbb{E}(M_{t} | \mathcal{F}_{s})$ p.s.
    \end{itemize}
\end{definitionbox}
...
\remark On voit bien ici que le prix est exprimé en fonction de $B_t$ qui devient ainsi le numéraire. \\
...
90 \; \text{avec probabilité $1-p$} \end{cases} \quad \text{avec } r = 5\%
\]

On cherche la probabilité risque-neutre $q$ telle que le prix actualisé soit une martingale sous $\mathbb{Q}$ :
$$
\tilde{S}_0 = \frac{1}{1+r}\mathbb{E^Q}[S_1] = \frac{120q + 90(1-q)}{1+r}
$$

La solution $q$ correspond à la \textbf{probabilité risque-neutre} de hausse de l'actif.
...
Le prix initial est :
$$
\Pi_0 = \frac{\mathbb{E}^{\mathbb{Q}}(\Pi_T)}{1+r} = \frac{1}{1+r} \big(20q + (1-q) \cdot0 \big) = \frac{20q}{1+r}
$$
$q$ est ici la probabilité risque-neutre de hausse de l'actif.  
